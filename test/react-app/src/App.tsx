...
import type { Api, Rpc, Todo, User, TodoStats } from 'virtual:effect/client'
...
function formatClientError(error: unknown) {
	...
	if (error && typeof error === 'object') {
		...
		candidates.push(record.error, record.cause)
	}

	for (const candidate of candidates) {
		if (candidate && typeof candidate === 'object') {
			const record = candidate as Record<string, unknown>
			if (typeof record._tag === 'string' || typeof record.code === 'string') {
				return record
			}
		}
	}
...
if (typeof window !== 'undefined') {
	;(window as unknown as {
		__vitePluginEffectSmoke?: {
			getTodosWithPromiseClient: () => Promise<readonly Todo[]>
			getTodosWithEffectClient: () => Promise<readonly Todo[]>
			getStatsWithPromiseClient: () => Promise<TodoStats>
			getStatsWithEffectClient: () => Promise<TodoStats>
			createInvalidTodoWithPromiseClient: () => Promise<{ ok: true } | { ok: false; error: unknown }>
		}
	}).__vitePluginEffectSmoke = {
		getTodosWithPromiseClient: () => promiseClient.api.todos.getTodos(),
		getTodosWithEffectClient: () => Effect.runPromise(effectClient.api.todos.getTodos()),
		getStatsWithPromiseClient: () => {
			const payload: Rpc.todoStats.Payload = {}
			return promiseClient.rpc.todoStats(payload)
		},
		getStatsWithEffectClient: () => {
			const payload: Rpc.todoStats.Payload = {}
			return Effect.runPromise(effectClient.rpc.todoStats(payload))
		},
		createInvalidTodoWithPromiseClient: async () => {
			const payload: Api.todos.createTodo.Payload = { title: '' }
			try {
				await promiseClient.api.todos.createTodo(payload)
				return { ok: true }
			} catch (error) {
				return { ok: false, error: formatClientError(error) }
			}
		},
	}
}
...
export default function App() {
	...
	const loadData = async () => {
		...
		try {
			...
			hasError = true
			addLog({ type: 'http', method: 'GET', endpoint: '/api/todos', request: {}, response: formatClientError(e), duration: Math.round(performance.now() - start), status: 'error' })
		}
		...
		try {
			...
			hasError = true
			addLog({ type: 'http', method: 'GET', endpoint: '/api/users', request: {}, response: formatClientError(e), duration: Math.round(performance.now() - userStart), status: 'error' })
		}
		...
		try {
			...
			hasError = true
			addLog({ type: 'rpc', method: 'RPC', endpoint: 'todoStats', request: {}, response: formatClientError(e), duration: Math.round(performance.now() - rpcStart), status: 'error' })
		}
	...
	const addTodo = async (title: Api.todos.createTodo.Payload['title']) => {
		const start = performance.now()
		...
		try {
			...
			addLog({ type: 'http', method: 'POST', endpoint: '/api/todos', request: { title }, response: formatClientError(e), duration: performance.now() - start, status: 'error' })
		} finally {
	...
	const addUser = async (name: string, email: string) => {
		...
		try {
			...
			addLog({ type: 'http', method: 'POST', endpoint: '/api/users', request: { name, email }, response: formatClientError(e), duration: performance.now() - start, status: 'error' })
		} finally {
			setLoading(prev => ({ ...prev, addUser: false }))
		}
	}

	const toggleTodo = async (id: Rpc.toggleTodo.Payload['id']) => {
		const start = performance.now()
		...
		try {
			...
			addLog({ type: 'rpc', method: 'RPC', endpoint: 'toggleTodo', request: { id }, response: formatClientError(e), duration: Math.round(performance.now() - start), status: 'error' })
		} finally {
	...
	const deleteTodoHttp = async (id: number) => {
		...
		try {
			...
			addLog({ type: 'http', method: 'DELETE', endpoint: `/api/todos/${id}`, request: {}, response: formatClientError(e), duration: Math.round(performance.now() - start), status: 'error' })
		} finally {
	...
	const updateTodoHttp = async (id: number, title: string) => {
		...
		try {
			...
			addLog({ type: 'http', method: 'PATCH', endpoint: `/api/todos/${id}`, request: { title }, response: formatClientError(e), duration: Math.round(performance.now() - start), status: 'error' })
		} finally {
	...
	const deleteUserHttp = async (id: number) => {
		...
		try {
			...
			addLog({ type: 'http', method: 'DELETE', endpoint: `/api/users/${id}`, request: {}, response: formatClientError(e), duration: Math.round(performance.now() - start), status: 'error' })
		} finally {
	...
	const updateUserHttp = async (id: number, name: string, email: string) => {
		...
		try {
			...
			addLog({ type: 'http', method: 'PATCH', endpoint: `/api/users/${id}`, request: { name, email }, response: formatClientError(e), duration: Math.round(performance.now() - start), status: 'error' })
		} finally {
	...
	const deleteTodoRpc = async (id: number) => {
		...
		try {
			...
			addLog({ type: 'rpc', method: 'RPC', endpoint: 'deleteTodo', request: { id }, response: formatClientError(e), duration: Math.round(performance.now() - start), status: 'error' })
		} finally {
	...
	const updateTodoRpc = async (id: number, title: string) => {
		...
		try {
			...
			addLog({ type: 'rpc', method: 'RPC', endpoint: 'updateTodo', request: { id, title }, response: formatClientError(e), duration: Math.round(performance.now() - start), status: 'error' })
		} finally {
